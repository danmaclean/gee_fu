  ...
  xml.DASGFF do
    xml.GFF 'version' => '1.0' do
        xml.SEGMENT 'id' => "seg1" do
            # logger.error"-----------------------------#{seq}"
            @features = Feature.where(experiment_id: @eid, feature: ["five_prime_UTR", "exon", "intron","three_prime_UTR"]).take(1000)
            ...
            @features.each do |feature|
              xml.FEATURE 'id' => feature.id, 'label' => feature.feature do
                ...
                logger.error "found #{Parent.where(parent_feature: 3325).length} parents with id of 3325"
