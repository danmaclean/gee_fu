class RegistrationsController < Devise::RegistrationsController
	...
	def create
		...
		webApolloOutputThree = %x(sudo -u postgres #{WebApolloPath}/tools/user/set_track_permissions.pl -D #{WebApolloDatabase} -U #{WebApolloDatabaseUsername} -P #{WebApolloDatabasePassword} -r -w -u #{email} -t #{WebApolloPath}/data/scratch/seqids.txt | echo)
